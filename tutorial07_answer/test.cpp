...
static void test_parse_null() {
	...
	EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "null")); // 成功解析null
	EXPECT_EQ_INT(LEPT_NULL, lept_get_type(&v)); // 看看类型是不是null
	lept_free(&v);
}
...
static void test_parse_true() {
	...
	lept_init(&v);
	lept_set_boolean(&v, 0);
	EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "true")); // 成功解析true
	EXPECT_EQ_INT(LEPT_TRUE, lept_get_type(&v)); // 看看类型是不是TRUE
	lept_free(&v);
}
...
static void test_parse_false() {
	...
	EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "false")); // 成功解析false
	EXPECT_EQ_INT(LEPT_FALSE, lept_get_type(&v)); // 看看类型是不是FALSE
	lept_free(&v);
}
